...
uv run pyinstaller --clean .\ingest.spec 2>&1 | Tee-Object -FilePath logs\pyinstaller.log
Write-Output "Creating setup package ..."
Write-Output "&  'C:\Program Files (x86)\Inno Setup 6\ISCC.exe' ..\ingest_installer.iss"
& 'C:\Program Files (x86)\Inno Setup 6\ISCC.exe' ..\ingest_installer.iss 2>&1 | Tee-Object -FilePath logs\inno.log