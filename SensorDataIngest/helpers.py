...
def load_data(contents, filename):
    ...
    Parameters:
        contents (str) Base64-encoded string of the file contents
        filename (str) The name of the file; only needed to get the extension
...
def multi_df_to_excel(df_data, df_meta, df_site):
    ...
    Parameters:
        df_data (DataFrame) Time sequence of multiple columns of sensor data
        df_meta (DataFrame) Column names and descriptions
        df_site (DataFrame) Site information
...
def ts_is_regular(timeseries, interval_minutes=15):
    ...
    Parameters:
        timeseries       (Series) Sequence of Datetime values
        interval_minutes (int)    Expected interval, in minutes, between consecutive values
...
def seqno_is_regular(seqno_series):
    ...
    Parameters:
        seqno_series (Series) Sequence of integers
